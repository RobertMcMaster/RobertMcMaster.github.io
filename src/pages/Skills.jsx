import { Database, BarChart2, Code2, Cloud, GitBranch, Zap } from "lucide-react";
    ...
    <div className="min-h-screen pt-20 px-4 max-w-6xl mx-auto pb-20">
      ...
      <ScrollAnimation>
        <p className="text-gray-400 mb-12 max-w-2xl">
          A focused overview of my technical expertise across data architecture,
          business intelligence, and programming.
        </p>
      </ScrollAnimation>
            ...
            <div className="bg-gray-800/50 p-6 rounded-lg backdrop-blur-sm hover:bg-gray-800/70 transition-all border border-white/5 h-full">
              ...
              <div className="grid grid-cols-2 gap-3">
                {skillGroup.items.map((skill) => (
                  <div
                    key={skill.name}
                    className="bg-gray-700/50 px-3 py-3 rounded-lg flex items-center gap-2 hover:bg-white/10 transition-all group"
                  >
                    <div className="text-gray-400 group-hover:text-white transition-colors flex-shrink-0">
                      {skill.icon}
                    </div>
                    <span className="text-gray-400 group-hover:text-white transition-colors text-sm leading-tight">
                      {skill.name}
                    </span>
                  </div>
                ))}
